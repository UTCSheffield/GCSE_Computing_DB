DROP TABLE stock; 
CREATE TABLE stock (
    stockid INTEGER PRIMARY KEY ASC, 
    name STRING, 
    max INTEGER
);
...
SELECT * FROM stock; --Select all the fields from every record
...
SELECT 
    ...
    stock.max - SUM(sales.quantity) as available 
FROM sales JOIN stock ON(stock.stockid = sales.stockid)  
